...
class Hr_group extends MY_Controller {
			...
			if( $this->form_validation->run() ) 
			{
				...
				if( $inserted_id = $this->hr_group_model->create_hr_group( $this->input->post('group_name'), $this->input->post('group_description'), $this->input->post('functional_area') ) ) 
				{
					if ($this->hr_group_model->create_hr_group_mapping( $inserted_id, $this->input->post('members') ) )
					{
						...
						//insert log
						$this->hr_group_model->insert_log_history( (int)$this->session->userdata('user_id'), 'HR Group', 'New HR Group \''.$this->input->post('group_name').'\' is created' );
						
						$this->session->set_flashdata('message', 'HR Group has been created.');
						redirect( 'admin_masters/hr_group');
				}
				else 
				{
					//$data['message'] = "Failed to crate the role";
					$this->session->set_flashdata('message', 'HR Group already exists. Please try again.');
		...
		if( count($_POST) ) {
			...
			if( $this->form_validation->run() ) {
				...
				if( $this->hr_group_model->update_hr_group( $this->input->post('group_name'), $this->input->post('group_description'), $this->input->post('functional_area'), $this->input->post('group_id') ) ) {
					...
					//insert log
					$this->hr_group_model->insert_log_history( (int)$this->session->userdata('user_id'), 'HR Group', 'HR Group \''.$this->input->post('group_name').'\' is updated' );
	...
	public function delete_hr_group()
	{
		if( count($_POST) ) {
			...
			$old_data = $this->hr_group_model->getById( HR_GROUP, HR_GROUP_ID, $_POST['group_id'] );
			if ( $this->hr_group_model->delete_hr_group( $_POST['group_id'] ) ) {
				//$data['message'] = "Role deleted successfuly";
				//insert log
				$this->hr_group_model->insert_log_history( (int)$this->session->userdata('user_id'), 'HR Group', 'HR Group \''.$old_data->{HR_GROUP_NAME}.'\' is deleted' );
				$message = 'HR Group removed successfully.';
			}
