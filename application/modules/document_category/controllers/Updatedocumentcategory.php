		...
		if( count($_POST) ) {
			...
			if( $this->form_validation->run() ) {
				...
				if( $this->updatedocumentcategory_model->update( $this->input->post('doc_category'), $this->input->post('doc_description'), $this->input->post('doc_cat_id') ) ) {
					...
					//insert in log
					$this->updatedocumentcategory_model->insert_log_history( (int)$this->session->userdata('user_id'), 'Document Category', 'Document Category - \''.$this->input->post('doc_category').'\' is updated' );
