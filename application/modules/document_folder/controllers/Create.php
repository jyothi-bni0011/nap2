		...
		if( count($_POST) ) {
			...
			if( $this->form_validation->run() ) {
				...
				if( $this->create_model->create( $this->input->post('folder_name'), $this->input->post('folder_description') ) ) {
					...
					mkdir( dirname(APPPATH) . '/assets/documents/'.$this->input->post('folder_name'), 0777, true );
					
					//insert in log
					$this->create_model->insert_log_history( (int)$this->session->userdata('user_id'), 'Document Folder', 'New Document Folder - \''.$this->input->post('folder_name').'\' is created' );
					
					$this->session->set_flashdata('message', 'Document folder has been created.');
				}
				else {
					//$data['message'] = "Failed to crate the job position";
					$this->session->set_flashdata('message', 'Document Folder already exists. Please try again.');
