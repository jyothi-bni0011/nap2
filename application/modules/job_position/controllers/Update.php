		...
		if( count($_POST) ) {
			...
			if( $this->form_validation->run() ) {
				...
				if( $this->update_model->update( $this->input->post('job_code'), $this->input->post('job_description'), $this->input->post('department'), $this->input->post('position_id') ) ) {
					...
					//insert in log
					$this->update_model->insert_log_history( (int)$this->session->userdata('user_id'), 'Job Title', 'Job Title \''.$this->input->post('job_code').'\' is updated' );
					
					$this->session->set_flashdata('message', 'Job Title has been updated.');
					redirect( '/job_position', $data );
					exit;
				}
				else {
					//$data['message'] = "Failed to crate the role";
					$this->session->set_flashdata('message', 'Failed to update the job Title');
