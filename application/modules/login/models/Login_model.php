...
class Login_model extends MY_Model {
	...
	public function models_assigned_to_user($role_id)
	{
		...
		$this->db->where( sprintf('%s.%s', ROLE_MENU, STATUS), 1 );
	...
	public function authenticate( $username, $password )
	{
		...
		if( $user ) {
			...
			if ( $user->associate_id ) {
				$new_associate = $this->get_data_for_new_associate( '', $user->associate_id );
				$init_session['org_unit'] = $new_associate->organizational_unit;
				$init_session['job_title'] = $new_associate->job_title;
				$init_session['fun_area'] = $new_associate->functional_area;
				$init_session['dept'] = $new_associate->department;
			}
