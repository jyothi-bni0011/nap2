					...
					<tr>
						...
						<td width="50%">
							<?php if( $document->form_step_role_id === $this->session->userdata('role_id') ): ?>
									...
									<?php if( $_GET ): ?>
										<?php if ( $_GET['r'] == 'view' ): ?>
											<a href="<?php echo base_url('document/generate/index/' . $document->document_id . '/' . $document->user_id .'?r=view&from=1' ); ?>"><?php echo $document->document_title; ?></a>
										<?php else : ?>
											<a href="<?php echo base_url('document/generate/index/' . $document->document_id . '/' . $document->user_id .'?r=dashboard&from=1' ); ?>"><?php echo $document->document_title; ?></a>
										<?php endif; ?>
									<?php else: ?>
