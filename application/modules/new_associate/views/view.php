...
<?php if( $verify ) : if( $this->session->userdata('role_id') != 4 ) : ?>
	<div class="row">
		...
		<div class="col-md-6">
				...
				<div class="">
					<button type="submit" name="verify" value="1" class="btn btn-success" data-toggle="modal" data-target="#verify_document">Verify</button>
				</div>
...
<div class="modal fade" id="verify_document" tabindex="-1" role="dialog">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<form class="modal-content" id="modal-form" method="post" action="<?php echo current_url(); ?>" autocomplete="off">
...
<script>
	$(document).ready(function(){
		...
		$("#reject_document").on("hidden.bs.modal", function(){
			$('#comment').val('');
		    validator.resetForm();
		});
	});
