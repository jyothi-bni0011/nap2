                    	...
                    	<div class="col-md-12">
                    		...
                    		<div class="card  card-box">
                    			...
                    			<div class="card-body ">
                    				...
                    				<a href="<?php echo base_url('admin_masters/map_document_job_position/create_map_document_job_position');?>"><button type="button" class="btn btn-success"><i class="fa fa-plus"></i>  Create Mapping </button></a>
                    					...
                    					<table id="tableExport" class="display table table-hover table-checkable order-column m-t-20" style="width: 100%">
                                         ...
                                         <tbody>
                                         	<?php $i=1; foreach($positions as $position): ?>
                                         		...
                                         		<tr class="odd gradeX">
                                                    <td><?= $i++;?></td>
                                         			<td><?php echo $position->{JOB_POSITION_CODE}; ?></td>
                                                    <td>
                                                        <?php if(array_key_exists($position->{JOB_POSITION_ID}, $documents) ): ?>
                                                        <?php foreach ($documents[$position->{JOB_POSITION_ID}] as $document): ?>
                                                            <?php echo $document->{DOCUMENT_TITLE}; ?><br/>
                                                        <?php endforeach; ?>
                                                        <?php endif; ?>
                                                    </td>
                                         			<td>
                                         				<a href="<?php echo base_url('admin_masters/map_document_job_position/update_map_document_job_position/').$position->{JOB_POSITION_ID};?>" class="btn btn-success btn-tbl-edit btn-xs">
                                                            <i class="fa fa-pencil"></i>
                                                        </a>
                                                        <button class="btn btn-tbl-delete btn-circle del_mapping" data-position_id="<?php echo $position->{JOB_POSITION_ID}; ?>" >
         ...
         <script type="text/javascript" >
         	...
         	$(document).ready(function(){
         		...
         		$('.position_id').on('click', function(){
         			...
         			$.ajax({
         				type: "POST",
         				url: '<?php echo base_url('admin_masters/map_document_job_position/delete_map_document_job_position');?>',
         				data: {position_id:id},
         				success:function(data) {
         					console.log(data);
         					if(data.success == "1") {
         						window.location.href = "<?php echo base_url('admin_masters/map_document_job_position/'); ?>";
         					}
         				},
         				error:function()
         				{
