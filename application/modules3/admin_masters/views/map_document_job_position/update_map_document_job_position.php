							...
							<div class="col-md-5">
								<select name="doc_folder" class="form-control input-height" id="doc_folder">
									...
									<?php foreach($doc_folders as $doc_folder): ?>
			                           <option value="<?php echo $doc_folder->{DOCUMENT_FOLDER_ID}; ?>" <?php foreach($rows as $mapped_doc): ?> <?php if ($doc_folder->{DOCUMENT_FOLDER_ID}==$mapped_doc->{DOCUMENT_FOLDER_ID}) echo 'selected'; ?> <?php endforeach; ?> > <?php echo $doc_folder->{DOCUMENT_FOLDER_NAME}; ?> </option>
		                       		<?php endforeach; ?>
								</select> 
							</div>
						</div>
