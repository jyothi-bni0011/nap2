			...
			if ( $decline ) {
				...
				if ( $this->document_update_model->insert_comment( $this->input->post('associate_id'),$this->input->post('document_id'), $this->input->post('comment') ) ) {
					...
					if( $updated ) {
						...
						if ( $this->document_update_model->send_email( 'rejected_document', 'venkatesh.jewlikar@talentserv.co.in', $values ) ) {
				...
				if( $updated ) {
					if( $this->document_update_model->all_document_verified( $this->input->post('associate_id') ) )
					{
						...
						if ( $this->document_update_model->send_email( 'verified_document', 'venkatesh.jewlikar@talentserv.co.in', $values ) )//for email address use $value->email variable
						{
