								...
								<?php foreach( $menues as $menu ): ?>
									<div class="form-check">
									  <label class="form-check-label">
									    <input type="checkbox" class="form-check-input" name="menu[]" value="<?= $menu->{MENU_ID} ?>" <?php foreach( $assigned_menues as $assign_menu ){ if($assign_menu->{MENU_ID} == $menu->{MENU_ID}) echo 'checked'; } ?> <?php if($menu->{MENU_ID} == 1) : ?> checked onclick="return false;" <?php endif; ?> ><?= $menu->{MENU_NAME} ?>
									  </label>
									</div>
								<?php endforeach; ?>
							</div>
